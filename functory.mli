...
module Cores : sig
  ...
  (** {1 Default API, tasks execution order is unspecified.} *)
  ...
  module type TASKQUEUE = sig
    ...
    val is_empty : t -> bool
    val pop : t -> data * id
    val push : data * id -> t -> unit
  end

  (** Functor building an implementation using [TQ] as queue for tasks
      execution order.

      [TQ] can be e.g. a priority queue. It is not used to prioritize
      simultaneous tasks scheduling, but to give an order in which
      tasks have to be processed. *)
  module Make(Q: TASKQUEUE) : sig

    type task = Q.data * Q.id

    val set_number_of_cores : int -> unit
    (** [set_number_of_cores n] indicates that [n] computations can be
        done in parallel. It is typically less or equal to the number
        of actual cores on the machine, though it is not mandatory.
        Setting [n] to 1 is equivalent to a sequential execution
        (though the order in which tasks are performed may differ). *)

    (** For documentation, refer to module {!Sequential}. *)
    val compute :
      worker:(Q.data -> 'a) ->
      master:(task -> 'a -> task list) -> task list -> unit
